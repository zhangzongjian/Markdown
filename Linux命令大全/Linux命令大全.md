...
用于向服务器发送 HTTP 请求，并获取响应（除了http，还支持file、ftp、ftps、https等协议）

| 参数 | 描述                       | 样例                                                           | 样例说明                                 |
| ---- | -------------------------- | -------------------------------------------------------------- | ---------------------------------------- |
|      | 发送请求                   | curl URL                                                       |                                          |
| -L   | 跟随重定向                 |                                                                | 如果请求存在重定向，会输出重定向后的结果 |
| -o   | 保存为文件，指定文件名     | curl URL -o filename                                           | 等同于wget URL -O filename               |
| -O   | 保存为文件，保留远程文件名 | curl URL -O                                                    | 即URL中/的最后一段作为文件名             |
| -s   | 不显示进度信息或错误消息   |                                                                | 等同于curl URL 2>/dev/null               |
| -H   | 设置请求头                 | -H "Content-Type:application/json"                             |                                          |
| -X   | 指定HTTP请求的方法         | -X POST                                                        |                                          |
| -i   | 显示结果包含响应头         |                                                                |                                          |
| -I   | 只显示响应头               |                                                                |                                          |
| -d   | 设置请求参数或者请求体     | -d "user=admin&pwd=123"<br />-d '{"user":"admin","pwd":"123"}' | 配合json请求头发送json数据               |
| -T   | 上传文件                   | curl FTP_URL -T /tmp/a.txt                                     | 上传文件到FTP                            |
| -F   | 上传文件                   | -F "param1=@/tmp/a.txt" -F "param2=value"                      | multipart请求，@代表文件参数             |
| -u   | 设置服务器的用户和密码     | -u "admin:pwd"                                                 |                                          |
| -#   | 显示进度条                 | curl FTP_URL -T /tmp/a.txt -#                                  |                                          |
